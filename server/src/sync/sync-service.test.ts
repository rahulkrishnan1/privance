...
describe("SyncService.put", () => {
  ...
  it("logs an audit event on success", async () => {
    ...
    expect(repo.logEvent).toHaveBeenCalledWith({ userId: "user-a", event: "sync.put" });
  });
});
...
describe("SyncService.delete", () => {
  ...
  it("calls repo.delete and logs event on success", async () => {
    ...
    expect(repo.logEvent).toHaveBeenCalledWith({ userId: "user-a", event: "sync.delete" });
  });
...
describe("SyncService.batch", () => {
  ...
  it("logs audit event for batch", async () => {
    (repo.batchPut as ReturnType<typeof mock>).mockResolvedValue([]);
    (repo.batchDelete as ReturnType<typeof mock>).mockResolvedValue([]);

    await service.batch({ userId: "user-a", puts: [], deletes: [] });
    expect(repo.logEvent).toHaveBeenCalledWith({ userId: "user-a", event: "sync.batch" });
  });
});
