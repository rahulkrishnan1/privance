...
describe("HKDF-SHA-256 — known-output vector (drift detection)", () => {
  it("produces a stable output for fixed inputs (snapshot catches parameter drift)", () => {
    ...
    const okm = hkdf(sha256, ikm, salt, info, 82);
    expect(bytesToHex(okm)).toMatchSnapshot();
  });
});
