...
describe("computeNetWorth, investments + holdings", () => {
  it("includes cash sweep + holding market value in investment bucket", () => {
    ...
    const input: NetWorthInput = { accounts, holdings, prices };
    const bd = computeNetWorth(input);
...
describe("computeNetWorth, proxy-priced holdings", () => {
  ...
  it("does not throw when scaleFactor has more decimal places than price scale (5dp)", () => {
    ...
    // 100 × (500.00 × 0.98765) → 100 × 493.83 = $49,382.50 → rounded to cents
    const bd = computeNetWorth({ accounts, holdings, prices });
    expect(bd.unknownTickers).toHaveLength(0);
    // Just verify it doesn't throw and returns a non-zero investment value
    expect(bd.byAccountKind.investment.isZero()).toBe(false);
  });
...
describe("computeNetWorth, primary currency is order-independent", () => {
  it("produces the same unknownTickers regardless of account order", () => {
    ...
    const bd1 = computeNetWorth({ accounts, holdings: [], prices: new Map() });
    const bd2 = computeNetWorth({ accounts: reversed, holdings: [], prices: new Map() });
...
describe("computeNetWorth, cents-string formats", () => {
  ...
  it("rejects malformed cents string", () => {
    ...
    const prices = priceMap({ AAPL: "200.00" });
    expect(() => computeNetWorth({ accounts, holdings, prices })).toThrow(/invalid cents string/);
  });
});
...
describe("computeNetWorth, asOf", () => {
  it("asOf is a recent ms epoch", () => {
    const before = Date.now();
    const bd = computeNetWorth({ accounts: [], holdings: [], prices: new Map() });
    const after = Date.now();
    expect(bd.asOf).toBeGreaterThanOrEqual(before);
    expect(bd.asOf).toBeLessThanOrEqual(after);
  });
});
  ...
  it("cash zero account", () => {
    fc.assert(
      fc.property(fc.array(arbCents, { minLength: 1, maxLength: 5 }), (balances) => {
        ...
        const baseNw = computeNetWorth({ accounts: baseAccounts, holdings: [], prices: new Map() });

        const withZero: Account[] = [...baseAccounts, makeCash("ca-zero", "0")];
        const withZeroNw = computeNetWorth({ accounts: withZero, holdings: [], prices: new Map() });
  ...
  it("holds for cash-only inputs", () => {
    fc.assert(
      fc.property(fc.array(arbCents, { minLength: 1, maxLength: 5 }), (balances) => {
        ...
        const bdBase = computeNetWorth({ accounts: base, holdings: [], prices: new Map() });
        const bdDoubled = computeNetWorth({ accounts: doubled, holdings: [], prices: new Map() });
    ...
    fc.assert(
      fc.property(
        ...
        (items) => {
          ...
          const bd1 = computeNetWorth({ accounts, holdings, prices });
          const bd2 = computeNetWorth({ accounts, holdings: reversed, prices });
...
describe("allocationBySector", () => {
  it("groups holdings by sector label", () => {
    ...
    computeNetWorth({ accounts, holdings, prices });
