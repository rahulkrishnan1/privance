...
// ---------------------------------------------------------------------------
// Hoisted spy refs (vi.mock factories run before imports)
// ---------------------------------------------------------------------------
...
describe("biometric section visibility", () => {
  it("hides the section entirely when isBiometricSupported returns false (AE1)", async () => {
    webauthnMock.isBiometricSupported.mockResolvedValue(false);
    const screen = await render(<SettingsPage />);

    // Wait for the async support check to settle (section must never appear).
    await vi.waitFor(async () => {
      await expect
        .element(screen.getByRole("region", { name: "Biometric unlock" }))
        .not.toBeInTheDocument();
    });

    // The Settings heading is present (page rendered correctly).
    await expect.element(screen.getByRole("heading", { name: "Settings" })).toBeVisible();
  });

  it("shows the section with an enroll button when supported and no record exists", async () => {
    const screen = await render(<SettingsPage />);

    await vi.waitFor(async () => {
      await expect.element(screen.getByRole("region", { name: "Biometric unlock" })).toBeVisible();
    });
