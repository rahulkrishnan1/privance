...
| Path | Purpose |
|---|---|
| `src/main.tsx` | Entry: QueryProvider > AuthProvider > SyncProvider > RouterProvider |
| `src/router.tsx` | Route tree (createBrowserRouter); app + auth + landing layouts |
| `src/app/(landing)/page.tsx` | Landing at `/` (auth-aware redirect for signed-in users) |
| `src/app/(app)/layout.tsx` | Auth-gated shell with top bar + mobile tab bar (mounts under `/app/*`) |
...
**Service worker:** generated by `vite-plugin-pwa` (Workbox) at build time; precaches the full asset manifest (hashed chunks + SQLite WASM + KDF/sim workers). Registered only in production builds. No-skip-waiting update flow with the in-app update banner.
...
- E2E builds run with reduced-KDF v2 params (`VITE_PRIVANCE_KDF_REDUCED`),
...
- All five projects run locally and in CI. The reduced-KDF v2 params
  (`VITE_PRIVANCE_KDF_REDUCED=true`) keep WebKit's Argon2id auth flows
